...
```bash
pnpm add -D github:oube-studio/oube-release#v0.4.0
```
...
```jsonc
{
  ...
  "android": {
    ...
    "featureGraphic": "store-assets/play-feature-1024x500-{locale}.png", // 있으면 upload_screenshots 가 Play 그래픽도 올림. {locale} 은 언어 코드로 바뀜
  },
...
- `app/__screenshots/[scene].tsx`: init 이 생성합니다. `@oube/expo/screenshots` 의 `createScreenshotScene` 을 쓰므로
  ...
  앱 배경색을 넘겨 라우트의 빈 화면이 모달 뒤로 비치지 않게 합니다.
